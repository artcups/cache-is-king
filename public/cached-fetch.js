...
Services.cachedFetch = (function() {
  ...
  function fetch(endpoint, payload, options, callback, failCallback, cacheKey) {
    $.ajax({
      ...
      .done(function(data, a, req) {
        ...
        if (
          headers &&
          (headers.match(/application\/json/i) || headers.match(/text\//i))
        ) {
  ...
  function cachedFetch(endpoint, payload, options, callback, failCallback) {
    ...
    if (cached !== null && whenCached !== null) {
      ...
      if (age < expiry) {
        if (typeof callback === "function") callback(JSON.parse(cached));
        return;
      } else {
    ...
    fetch(endpoint, payload, options, callback, failCallback, cacheKey);
  }
  return {
